...
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb+srv://send2mazi:Mazyar4321@iknowme.spbc4cr.mongodb.net/iknowme?retryWrites=true&w=majority&appName=iKnowMe';
...
app.use((req, res, next) => {
  // If the request is for an API route, continue to the next middleware
...
io.on('connection', (socket) => {
  ...
  socket.on('createSession', async (data) => {
    ...
    try {
      ...
      // Get the origin from the socket handshake
      const origin = socket.handshake.headers.origin || process.env.FRONTEND_URL;
      
      const links = players.map(p => ({
        username: p,
        token: playerTokens[p],
        url: `${origin}/#/rate/${sessionId}/${playerTokens[p]}`
      }));

      const adminLink = `${origin}/#/rate/${sessionId}/admin`;
  ...
  socket.on('startTimer', async ({ sessionId }) => {
    ...
    try {
      ...
      setTimeout(handleTimerExpiration, session.timer * 1000); // Convert seconds to milliseconds
    } catch (error) {
...
server.listen(PORT, '0.0.0.0', () => {
  console.log(`DEBUG: Server running in ${NODE_ENV} mode on port ${PORT}`);
  console.log(`DEBUG: Allowed origins: ${allowedOrigins.join(', ')}`);
});
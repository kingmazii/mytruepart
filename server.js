  ...
  socket.on('createSession', async (data) => {
    ...
    try {
      ...
      const links = players.map(p => ({
        ...
        url: `${process.env.FRONTEND_URL || 'http://localhost:3000'}/#/rate/${sessionId}/${playerTokens[p]}`
      }));

      const adminLink = `${process.env.FRONTEND_URL || 'http://localhost:3000'}/#/rate/${sessionId}/admin`;
